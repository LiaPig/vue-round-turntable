  ...
  <div class="turntable" ref="turntable">
    <div class="myTurntable" :style="{transform: rotateAngle, transition: rotateTransition}">
      ...
      <div class="prize-container">
        <div v-for="(item, index) in prizeData" class="item" :style="{transform: `rotate(${360 / prizeData.length * index + (180 / prizeData.length)}deg)`, 'transform-origin': 'center bottom'}">
  ...
  export default {
    ...
    data() {
      return {
        ...
        startRotateDegree: 0,
        rotateAngle: 0,
        rotateTransition: "transform 4.5s cubic-bezier(0.250, 0.460, 0.455, 0.995)",
      }
    },
    methods: {
      // 初始化圆形转盘canvas
